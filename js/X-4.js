...
$(document).ready(function() {
	...
	getFilterValues();
        ...
        $("#query").html(condition).css("display", "hidden");
        ...
        $('#LogListContainer').jtable('load', {from:from, to:to, select:select, books:books.join(',')});
	    ...
	    actions: {
            listAction: function (postData, jtParams) {
                return $.Deferred(function ($dfd) {
                    $.ajax({
                        url: "ajax/X-4/LogList.php?from="+postData.from+"&to="+postData.to+"&select=" + postData.select + "&videos="+postData.books,
                        type: 'POST',
                        dataType: 'json',
                        data: postData,
            ...
            toolbar: {
                items: [{
                    ...
                    click: function () {
                            ...
                            getFilterValues();
                            l="ajax/X-4/toExcel.php?from="+from+"&to="+to+"&select=" + select + "&videos="+books.join(',');
                            location.href=l;
                    }
                }]
            },
...
/*
                    display: function(data) {
                        url = "B-18_showlog.php?books="+data.record.SERIAL_NUMBER+"&from="+from+"&to="+to;
