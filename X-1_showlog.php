...
<html>
	<head>
		...
		<link rel="stylesheet" href="css/X-1.css"> 
    ...
    document.getElementById(form_id).submit();
                ...
                $(document).ready(function() {
                    ...
                    $('#SingleLogListContainer').jtable({
                        ...
                        sorting: false,
                        actions: {
                            listAction: 'ajax/X-1/LogList.php'
                        },
...
/*
                        toolbar: {
                            items: [{
                                text: '匯出Excel檔',
                                click: function () {
                                        l="ajax/X-1/toExcel.php?from=<?php echo $from;?>&to=<?php echo $to; ?>&uid=<?php echo $uid;?>&select=<?php echo $select;?>&videos=<?php echo $videos;?>&units=<?php echo $units; ?>";
                                        location.href=l;
                                }
                            }]
                        },
*/
                    ...
                    var table = {
                        ...
                        sorting: true,
                        actions: {
                            listAction: 'ajax/X-1/LogList_video.php',
                        },
                        toolbar: {
                            items: [{
                                text: '匯出Excel檔',
...
/*
                            TOTAL_TIME: {
                                ...
                                width: '10%',
                                display: function (data) {
                                    return Math.ceil((parseInt(data.record.TOTAL_TIME)+1)/60);
                                },
                                list: true
                            },
*/
...
<?php echo $js_video_array_declare; ?> 
                    ...
                    var video_list = ["<?php echo implode('","', $video_array); ?>"];

/*
                    $.each(video_list, function(index, video_id) {

                        $('<h3 style="color:#44aaaa">'+ video_id +' ' +video_info[video_id]+'</h3>').appendTo('#LogListContainer');

                        $block = $('<div>');
                        $block.appendTo('#LogListContainer');
                        $block.jtable(table);

                        $block.jtable('load', {from:from, to:to, uid:uid, select:select, video:video_id, units:units, ranks:ranks, rank_select:rank_select});
...
*/
                    ...
                    serial_number=$("#video_list").val(); currentVideo = serial_number;

                    $('<h3 id="selected_title" style="color:#44aaaa">'+ serial_number +' ' +video_info["video"+serial_number]+'</h3>').appendTo('#LogListContainer');
                    $block = $('<div>');
                    $block.appendTo('#LogListContainer');
                    $block.jtable(table);

                    $block.jtable('load', {from:from, to:to, uid:uid, select:select, video:serial_number, units:units, ranks:ranks, rank_select:rank_select});
...
<button id="exportButton" onclick="doExport();">匯出全部Excel檔(需較長時間)</button>
            <h1> 閱讀記錄查詢結果 </h1>
    <form id='formid' method='post' action='ajax/X-1/toExcel2.php'>
