...
debug(print_r($_FILES['VideoFile'], true));
...
*/
            ...
            $duration = Video::getVideoDuration($v);
            $count = Video::getVideoCount($id);

            $video = new Video();
            $video->init();
            $video->updateField($id, "VIDEO_LENGTH", $duration);
            $video->updateField($id, "VIDEOS", $count);
